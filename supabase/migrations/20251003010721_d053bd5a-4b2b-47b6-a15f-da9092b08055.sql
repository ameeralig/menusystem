...
-- enum لحالة الطلب (مع التحقق من عدم وجوده)
DO $$ BEGIN
...
DROP POLICY IF EXISTS "Employees can update table status" ON public.tables;
...
DROP POLICY IF EXISTS "Employees can view store orders" ON public.orders;
CREATE POLICY "Employees can view store orders"
  ON public.orders FOR SELECT
  USING (public.is_active_employee(auth.uid(), store_owner_id));

DROP POLICY IF EXISTS "Employees can create orders" ON public.orders;
...
DROP TRIGGER IF EXISTS update_employees_timestamp ON public.employees;
...
DROP TRIGGER IF EXISTS trigger_calculate_order_total ON public.order_items;
