...
export const useCategoryImages = (userId: string | null, forceRefresh: number) => {
  ...
  const fetchCategoryImages = useCallback(async () => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        console.log("البيانات المستلمة لصور التصنيفات:", JSON.stringify(data));
  ...
  // تنفيذ جلب الصور عند تغيير المستخدم أو طلب التحديث
  useEffect(() => {
    fetchCategoryImages();
  }, [userId, forceRefresh, fetchCategoryImages]);

  // تنفيذ إعادة جلب الصور بشكل دوري (كل 30 ثانية)
  useEffect(() => {
    const intervalId = setInterval(() => {
      if (userId) {
        console.log("تنفيذ تحديث دوري لصور التصنيفات");
        fetchCategoryImages();
      }
    }, 30000);
    
    return () => clearInterval(intervalId);
  }, [userId, fetchCategoryImages]);

  return { categoryImages, isLoading, refetchCategoryImages: fetchCategoryImages };
};
