...
const AddProductModal = ({ isOpen, onOpenChange, onProductAdded, colorTheme }: AddProductModalProps) => {
  ...
  const fetchCategories = async () => {
    try {
      ...
      const { data: products } = await supabase
        .from("products").select("category").eq("user_id", user.id).not("category", "is", null);
      if (products) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (imageUploadState.uploadMethod === "file" && imageUploadState.selectedFile) {
        try {
          const result = await upload(imageUploadState.selectedFile, {
            bucket: 'product-images',
            folder: '',
            userId: user.id,
          });

          if (!result || !result.url) throw new Error("فشل في الحصول على رابط الصورة");
          imageUrl = result.url;
      ...
      const { error } = await supabase.from("products").insert([{
        ...
        price: parseFloat(formData.price), image_url: imageUrl,
        category: formData.category || null, user_id: user.id,
        is_new: formData.is_new, is_popular: formData.is_popular,
      ...
      logUserActivity('product_add', 'products', { name: formData.name, category: formData.category });
  ...
  const resetForm = () => {
    setFormData({ name: "", description: "", price: "", image_url: "", category: "",
      is_new: false, is_popular: false, discount_percentage: "", original_price: "", discount_method: "original_price" });
    setImageUploadState({ uploadMethod: "url", selectedFile: null, previewUrl: null });
    setCurrentStep(1);
  };
          ...
          <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }}
            ...
            <div className="pointer-events-auto w-full max-w-lg my-8">
              ...
              <div className="rounded-3xl overflow-hidden shadow-2xl border border-white/20"
                style={{ background: `linear-gradient(135deg, ${themeColor}ee, ${themeColor}cc)`, backdropFilter: 'blur(20px)' }}>
                ...
                <div className="relative p-6 text-center text-white border-b border-white/20">
                  ...
                  <motion.h2 initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
                    className="text-2xl font-bold drop-shadow-lg">إضافة منتج جديد</motion.h2>
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}
                    className="flex items-center justify-center gap-2 mt-4">
                    <div className={cn("flex items-center justify-center w-8 h-8 rounded-full font-bold text-sm transition-all",
                      currentStep >= 1 ? "bg-white text-primary shadow-lg" : "bg-white/30 text-white/70")}>1</div>
                    <div className={cn("h-1 w-12 rounded transition-all", currentStep >= 2 ? "bg-white" : "bg-white/30")} />
                    <div className={cn("flex items-center justify-center w-8 h-8 rounded-full font-bold text-sm transition-all",
                      currentStep >= 2 ? "bg-white text-primary shadow-lg" : "bg-white/30 text-white/70")}>2</div>
                  </motion.div>
                </div>
                <div className="relative p-6 bg-white dark:bg-gray-900 max-h-[60vh] overflow-y-auto">
                  {currentStep === 1 ? (
                    <CategorySelectionStep categories={categories} selectedCategory={formData.category}
                      onCategorySelect={(category) => setFormData({ ...formData, category })}
                      onNext={() => setCurrentStep(2)} onCategoriesUpdate={setCategories} />
                  ) : (
                    <ProductDetailsStep formData={formData} onFormDataChange={setFormData}
                      onBack={() => setCurrentStep(1)} onSubmit={handleSubmit} loading={loading}
                      imageUploadState={imageUploadState} onImageUploadStateChange={setImageUploadState} />
                  )}
                </div>
              </div>
            </div>
